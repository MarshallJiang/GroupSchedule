...
class OfferGroupsCursor :
    def __init__(self, offer_id, aff_id=1059, branch_at=None) :
        ...
        self.groups = self.__get_id_list(branch_at)    

    def __get_id_list(self, branch_at=None) :
        if branch_at :
            ...
            offer_id = '%s' AND 
            actived_from <= '%s' AND 
            actived_to > '%s');'''%(self.offer_id, self.offer_id, branch_at, branch_at))          
        ...
        self.groups = {x['cashflow_group_id'] : Group(self.offer_id, x['cashflow_group_id'], x['percent'], x['rate'], rules=json.loads(x['rules']) if branch_at else None) for x in raw_group}
    ...
    def group_append(self, cashflow_group_id, percent=None, rate=None) : 
        ...
        self.groups[str(cashflow_group_id)] = Group(self.offer_id, cashflow_group_id, percent, rate)
    ...
    def push_cursor(self, utc=8) :
        ...
        for cashflow_group_id, group in self.groups.items() : 
            ...
            columns = "(%s)"%','.join(group.__dict__.keys())
            values.append("('%s')"%("','".join(str(x).replace("'", '"') for x in group.__dict__.values())))
        ...
        for i, j in {'replaceOfferPayoutGroupsForOffer':'offer_payout_groups_json', 'replaceOfferRevenueGroupsForOffer':'offer_revenue_groups_json'}.items() : #attach groups to offer.
            r = api_req(api_url, [
                ...
                join(['{"cashflow_group_id":"%s" %s %s}'%(
                    ...
                    '' if not x.rate and self.ptype not in ('cpa_rate', 'cpa_both') else ',"rate":"%s"'%(x.rate if x.rate else 0)) 
    ...
    def __attach_value(self) :
        for j in ['percent_payout', 'max_payout', 'max_percent_payout', 'default_payout'] : #setup value for group
            if (self.ptype == 'cpa_percentage' and j in ('max_payout', 'default_payout')) or (self.ptype == 'cpa_rate' and j in ('percent_payout','max_percent_payout')) :
                continue
            r = api_req(api_url, [
                {'Target':'Offer'},
                {'Method':'updateField'},
    ...
    def __attach_schedule(self, utc) :
        ...
        for t in [self.actived_from, self.actived_to] :
            for g in ['percent_payout', 'max_payout', 'max_percent_payout', 'default_payout'] :
                if (self.ptype == 'cpa_percentage' and g in ('max_payout', 'default_payout')) or (self.ptype == 'cpa_rate' and g in ('percent_payout','max_percent_payout')) :
                    continue
                r = api_req(api_url, [
                    config_params,
...
class Group :
    ...
    '''
    def __init__(self, offer_id, cashflow_group_id, percent, rate, follow=1, rules=None, is_base=False) :
        if not is_base :
            r = api_req(api_url, [
                config_params,
                {'Target':'CashflowGroup'},
...
def overlap_revised(t, offer_id, revises) : 
    cursor = database.cursor(pymysql.cursors.DictCursor)
    flag = False
    for r in revises :  #check follow event active/ close in the moment.
        cursor.execute('''SELECT cashflow_group_id, rate, percent FROM GroupSchedule WHERE 
        offer_id = '%s'
        AND cashflow_group_id = '%s'
        AND created_at = (
        SELECT MAX(created_at) 
        FROM GroupSchedule 
        WHERE ((actived_from BETWEEN '%s' AND '%s') OR (actived_to BETWEEN '%s' AND '%s' ))
        AND status = 'pending'
        AND follow = 0
        AND offer_id = '%s')
        '''%(offer_id, r, t-datetime.timedelta(hours=1)+datetime.timedelta(seconds=1), t, t-datetime.timedelta(hours=1)+datetime.timedelta(seconds=1), t, offer_id))
        c = cursor.fetchone()
        if c : #get highest priority pending group within eligible period, same cashflow_group_id and follow = 0.
            cursor.execute('''SELECT cashflow_group_id, rate, percent FROM GroupSchedule WHERE 
            created_at = (SELECT max(created_at) 
            FROM GroupSchedule
            WHERE actived_from <= '%s' 
            AND actived_to >= '%s'
            AND offer_id = '%s'
            AND status = 'pending'
            AND (follow = 0 or group_type = 'base')
            AND cashflow_group_id = '%s'
            )
            AND cashflow_group_id = '%s'
            '''%(t, t, offer_id, r, r))
            ...
            AND offer_id = '%s'
            AND status = 'actived'
            '''%("'%s'"%o['percent'] if o['percent'] else 'null', "'%s'"%o['rate'] if o['rate'] else 'null', o['cashflow_group_id'], offer_id))
            print('''[Note] Offer %s - Group %s Has Been Adjusted As percent: %s , rate: %s'''%(offer_id, c['cashflow_group_id'], o['percent'], o['rate']))
            flag = True
    return flag
            
    
def job(even, context) :
    ...
    for group in groups :
        ...
        if patch :
            ...
            elif patch['status'] == 'actived' and patch['group_type'] != 'base' :
                revises = [p['cashflow_group_id'] for p in patches if p['follow'] == 1]
                o = overlap_revised(t, patch['offer_id'], revises)
                if o :
                    c = OfferGroupsCursor(patch['offer_id'], branch_at=t)
                    c.deploy_cursor()
